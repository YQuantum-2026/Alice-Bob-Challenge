...
na      = 12    # storage Fock-space truncation (reduced from 15 for speed)
nb      = 4     # buffer Fock-space truncation  (reduced from 5 for speed)
...
def measure_Tx(eps_d: float, g2: float,
    ...
    if tfinal is None:
        tx_est = analytic_Tx(eps_d, g2)
        tfinal = float(np.clip(3.0 * tx_est, 0.3, 15.0))
...
def measure_Tz(eps_d: float, g2: float,
    ...
    # Logical-Z operator in storage space (notebook Cell 38 convention)
    sz_s  = ket_p @ ket_p.dag() - ket_m @ ket_m.dag()
    sz_op = dq.tensor(sz_s, dq.eye(nb))

    # Initial state: |+z> = |+alpha> ⊗ |0>_b
    psi0_z = dq.tensor(ket_p, dq.fock(nb, 0))

    if tfinal is None:
        tz_est = analytic_Tz(eps_d, g2)
...
def measure_joint(eps_d: float, g2: float) -> dict:
    """
    Measure T_X and T_Z for given parameters.
    ...
    """
    out_x = measure_Tx(eps_d, g2)
    Tx    = out_x["T_X"]

    tz_est    = analytic_Tz(eps_d, g2)
    tz_window = float(np.clip(3.0 * tz_est, 50.0, 500.0))
    out_z = measure_Tz(eps_d, g2, tfinal=tz_window, n_pts=50)
    Tz    = out_z["T_Z"]
...
def optimize_joint(
    ...
    Loss = -(T_X + T_Z/TARGET_RATIO)/2
           + RATIO_PENALTY * T_X * ((T_Z/T_X - TARGET_RATIO)/TARGET_RATIO)^2
        ...
        for _ in range(batch_size):
            ...
            try:
                ...
                val = (-(Tx + Tz / TARGET_RATIO) / 2.0
                       + RATIO_PENALTY * Tx * ratio_err**2)
            except Exception:
                Tx, Tz, ratio, val = 0.0, 0.0, 0.0, 1e6

            batch_solutions.append((x, val))
            batch_metrics.append({"T_X": Tx, "T_Z": Tz,
                                   "ratio": ratio, "x": x, "loss": val})
...
if __name__ == "__main__":
    ...
    nb_sim_x = measure_Tx(NOTEBOOK_EPS_D, NOTEBOOK_G2,
                          tfinal=NOTEBOOK_TX_FINAL, n_pts=60)
