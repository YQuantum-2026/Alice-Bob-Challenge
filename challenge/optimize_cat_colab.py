...
# Simulates the simulation-to-reality gap (see challenge notebook §3):
#   1. Control noise:     g2, eps_d jittered each evaluation
#   2. Measurement noise: Gaussian added to <X_L>, <Z_L> expectation traces
# Set any sigma to 0 to disable that noise source.
NOISE_ENABLED = os.getenv("CAT_NOISE", "1") == "1"
NOISE_SIGMA_CONTROL = float(os.getenv("CAT_NOISE_CTRL", "0.01"))   # relative σ on g2, eps_d
NOISE_SIGMA_MEAS = float(os.getenv("CAT_NOISE_MEAS", "0.01"))      # absolute σ on expectation values
_NOISE_RNG = np.random.default_rng(int(os.getenv("CAT_NOISE_SEED", "42")))

# Cache (disabled when noise is on — identical params give different results)
...
def simulate(g2, eps_d, init_state, tfinal, n_points):
    ...
    # Measurement noise: i.i.d. Gaussian on each time-point
    if NOISE_ENABLED and NOISE_SIGMA_MEAS > 0:
        x_out = x_out + _NOISE_RNG.normal(0.0, NOISE_SIGMA_MEAS, size=x_out.shape)
        z_out = z_out + _NOISE_RNG.normal(0.0, NOISE_SIGMA_MEAS, size=z_out.shape)
...
def _noisy_controls(g2, eps_d):
    """Apply per-evaluation Gaussian jitter to control parameters.

    Returns (g2_noisy, eps_d_noisy).  When noise is disabled the inputs
    are returned unchanged.
    """
    if not NOISE_ENABLED or NOISE_SIGMA_CONTROL <= 0:
        return g2, eps_d
    g2 = g2 * (1.0 + _NOISE_RNG.normal(0.0, NOISE_SIGMA_CONTROL))
    eps_d = eps_d * (1.0 + _NOISE_RNG.normal(0.0, NOISE_SIGMA_CONTROL))
    return g2, eps_d
...
def measure_Tx_Tz(g2, eps_d, n_points=N_POINTS, tz_tfinal=TZ_TFINAL, tx_tfinal=TX_TFINAL):
    ...
    When noise is enabled the cache is bypassed so each call yields a
    fresh stochastic draw (different control / measurement noise).
    """
    # Deterministic cache path (noise off)
    if not NOISE_ENABLED:
        key = _measure_cache_key(g2, eps_d, n_points, tz_tfinal, tx_tfinal)
        if key in _MEASURE_CACHE:
            return _MEASURE_CACHE[key]

    # Apply per-call Gaussian control perturbations
    g2_n, eps_d_n = _noisy_controls(g2, eps_d)

    tz_t, _, sz = simulate(g2_n, eps_d_n, "+z", tz_tfinal, n_points=n_points)
    Tz = min(fit_decay(tz_t, sz), TZ_MAX)

    tx_t, sx, _ = simulate(g2_n, eps_d_n, "+x", tx_tfinal, n_points=n_points)
...
if __name__ == "__main__":
    ...
    default_rewards = list(REWARD_FUNCTIONS.keys())
    ...
    print(
        ...
        f"{'  ctrl_\u03c3=' + str(NOISE_SIGMA_CONTROL) + '  meas_\u03c3=' + str(NOISE_SIGMA_MEAS) if NOISE_ENABLED else ''}"
    )
